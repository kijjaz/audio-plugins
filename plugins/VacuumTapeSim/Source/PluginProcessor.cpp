...
juce::AudioProcessorValueTreeState::ParameterLayout VacuumTapeSimAudioProcessor::createParameterLayout()
{
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>("wow", "Wow", juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.0f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>("flutter", "Flutter", juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.0f));
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>("hiss", "Tape Hiss", juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.0f));
...
{
    // Preset definition:
    // name, drive, sag, ips, wow, flutter, bias, asymmetry, eq_mode (0:NAB, 1:CCIR), auto_gain, output, mix, hiss
    presets = {
        { "Default Clean 15 IPS",      1.0f, 0.40f, 15.0f, 0.00f, 0.00f,  0.0f, 0.00f, 0, true,   0.0f, 100.0f, 0.00f },
        { "Mastering Warmth (30 IPS)", 0.8f, 0.20f, 30.0f, 0.00f, 0.00f,  0.2f, 0.00f, 1, true,   0.0f, 100.0f, 0.00f },
        { "Vintage 7.5 IPS Dark",      1.5f, 0.50f,  7.5f, 0.15f, 0.08f, -0.3f, 0.05f, 0, true,   0.0f, 100.0f, 0.25f },
        { "Drum Bus Fat Sag",          2.8f, 0.75f, 15.0f, 0.02f, 0.01f,  0.1f, 0.10f, 0, true,  -1.0f,  85.0f, 0.05f },
        { "Hot Tube Overdrive",        5.5f, 0.85f, 15.0f, 0.04f, 0.02f,  0.0f, 0.25f, 0, true,  -2.5f, 100.0f, 0.00f },
        { "Wobbly Cassette Lo-Fi",     1.8f, 0.60f,  7.5f, 0.65f, 0.45f, -0.6f, 0.15f, 0, true,   0.0f, 100.0f, 0.45f },
        { "Analog Tape Flanger",       1.2f, 0.30f, 15.0f, 0.40f, 0.85f,  0.0f, 0.00f, 1, false,  0.0f,  50.0f, 0.10f },
        { "Subtle Parallel Glow",      3.2f, 0.65f, 30.0f, 0.02f, 0.01f,  0.3f, 0.08f, 1, true,   0.0f,  40.0f, 0.00f }
    };
}
...
void VacuumTapeSimAudioProcessor::loadPreset(int index)
{
    ...
    if (auto* param = apvts.getParameter("wow")) param->setValueNotifyingHost(param->convertTo0to1(p.wow));
    if (auto* param = apvts.getParameter("flutter")) param->setValueNotifyingHost(param->convertTo0to1(p.flutter));
    ...
    if (auto* param = apvts.getParameter("hiss")) param->setValueNotifyingHost(param->convertTo0to1(p.hiss));
}
...
void VacuumTapeSimAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    float flutter = apvts.getRawParameterValue("flutter")->load();
    ...
    float hiss = apvts.getRawParameterValue("hiss")->load();
    ...
    {
        tapeDSP[i].updateParameters(drive, sag, ips, bias, asym, wow, flutter, eq, hiss);
    }
